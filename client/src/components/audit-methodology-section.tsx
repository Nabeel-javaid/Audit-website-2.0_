...
const methodologySteps = [
    {
        ...
        icon: (
            <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 sm:w-10 sm:h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
const AuditMethodologySection = () => {
    ...
    return (
        <section id="methodology" ref={sectionRef} className="py-12 sm:py-16 md:py-24 relative overflow-hidden">
            <div className="container mx-auto px-4 sm:px-6">
                ...
                <div className="max-w-4xl mx-auto relative">
                    ...
                    <div className="absolute left-4 sm:left-6 md:left-1/2 top-0 bottom-0 w-0.5 transform md:-translate-x-1/2 z-0">
                    ...
                    {methodologySteps.map((step, index) => (
                        <motion.div
                            key={index}
                            className={`flex flex-col md:flex-row gap-4 sm:gap-6 md:gap-0 items-start relative mb-10 sm:mb-12 md:mb-16 last:mb-0 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''
                                }`}
                            variants={fadeIn(
                        ...
                        >
                            ...
                            <motion.div
                                className="absolute left-4 sm:left-6 md:left-1/2 top-8 sm:top-10 w-4 sm:w-6 h-4 sm:h-6 rounded-full bg-background transform md:-translate-x-1/2 z-10 flex items-center justify-center"
                                initial={{ scale: 0 }}
                                whileInView={{ scale: 1 }}
                                viewport={{ once: true }}
                            ...
                            >
                                ...
                                />
                                <motion.div
                                    className="absolute w-1.5 sm:w-2 h-1.5 sm:h-2 rounded-full bg-primary"
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                            ...
                            <motion.div
                                className={`absolute left-8 sm:left-12 md:left-auto ${index % 2 === 0 ? 'md:right-[calc(50%+1.5rem)]' : 'md:left-[calc(50%+1.5rem)]'
                                    } top-7 sm:top-9 text-base sm:text-xl font-mono font-bold text-primary opacity-0`}
                                animate={{
                                    opacity: [0, 0.3, 0.6],
                            ...
                            {/* Mobile spacer to push content to the right of timeline */}
                            <div className="w-10 sm:w-16 md:w-0 flex-shrink-0 md:hidden"></div>

                            {/* Content - alternating sides on desktop, stacked on mobile */}
                            <div className={`w-full pl-5 sm:pl-8 md:pl-0 md:w-[calc(50%-1.5rem)] ${index % 2 === 0 ? 'md:pr-8 lg:pr-12' : 'md:pl-8 lg:pl-12'
                                }`}>
                                {/* Step content with CSS-only hover effects for better performance */}
                                <div
                                    className="bg-card/20 backdrop-blur-sm p-4 sm:p-6 rounded-xl border border-white/5 transition-all duration-200 
                            ...
                            {/* Placeholder for the timeline on desktop */}
                            <div className="hidden md:block w-0 md:w-[3rem] lg:w-[6rem] flex-shrink-0"></div>
                        </motion.div>
                    ))}
                </div>
            </div>
